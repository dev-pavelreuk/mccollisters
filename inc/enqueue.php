...
/**
 * Load the store locator's stylesheets without blocking first paint.
 *
 * The homepage renders an [ASL_SEARCH] box, and Agile Store Locator enqueues
 * four stylesheets for it -- sl-bootstrap alone is 25KB. Lighthouse measured the
 * set at 33.7KB and ~1.1s of render-blocking time on its Slow 4G mobile
 * profile, second only to the theme's own CSS.
 *
 * None of it is needed for first paint here: the box is display:none below
 * 782px, and its JavaScript is consent-gated by Complianz (type="text/plain"),
 * so it does not even function until a visitor accepts marketing cookies.
 *
 * media="print" makes the browser fetch the file at low priority without
 * blocking render; the onload handler swaps it back to its real media once it
 * arrives. Scoped to the front page on purpose -- on the Locations archive the
 * locator IS the content and must be styled immediately.
 */
function mcc_defer_store_locator_styles(string $tag, string $handle, string $href, string $media): string
{
    if (!is_front_page()) {
        return $tag;
    }
    ...
    $deferred = [
        'agile-store-locator-init',
        'agile-store-locator-sl-icons',
        'agile-store-locator-sl-bootstrap',
        'agile-store-locator-asl-search',
    ];
...
add_filter('style_loader_tag', 'mcc_defer_store_locator_styles', 10, 4);
