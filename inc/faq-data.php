<?php
/**
 * FAQ content, and the one API every template reads it through.
 *
 * Templates call mcc_faqs_for('<location>'); /faqs/ also uses the General group
 * and mcc_industry_faqs(). Where the FAQs come from:
 *   - once any FAQ is published in WordPress (FAQs, inc/faq-cpt.php), from there;
 *   - until then, from mcc_faq_data() below -- the original set supplied by
 *     Julia in September 2026, which `wp mcc-faqs import` copies into WordPress.
 *
...
/**
 * URL of a location's FAQ PDF, by file name. The current set (regenerated by
 * Julia, September 2026) lives in the 2026/09 uploads folder.
 */
function mcc_faq_pdf_url(string $file): string
{
    return $file === '' ? '' : trailingslashit(wp_get_upload_dir()['baseurl']) . '2026/09/' . $file;
}

/**
 * The original data, render-ready: tokens resolved, PDF URLs built.
 */
function mcc_faq_groups_legacy(): array
{
    $groups = mcc_faq_data();

    foreach ($groups as $slug => $group) {
        $groups[$slug]['pdf_url'] = mcc_faq_pdf_url($group['pdf'] ?? '');
...
/**
 * Every FAQ group, keyed by slug: ['label', 'pdf_url', 'items' => [['q', 'a'], ...]].
 *
 * 'general' holds the FAQs with no location -- the main list on /faqs/.
 */
function mcc_faq_groups(): array
{
    ...
    if ($groups === null) {
        $groups = (function_exists('mcc_faq_groups_from_db') ? mcc_faq_groups_from_db() : null)
            ?? mcc_faq_groups_legacy();
    }
...
function mcc_industry_faqs(): array
{
    ...
    // A location with no FAQs would otherwise list an empty pop-up.
